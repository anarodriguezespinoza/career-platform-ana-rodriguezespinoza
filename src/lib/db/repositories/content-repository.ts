...
export class ContentRepository {
  ...
  async saveDraft(input: SaveDraftInput): Promise<EditableContent> {
    ...
    if (input.type === "profile") {
      ...
      await this.db.project.upsert({ where: { id: input.id }, create: { id: input.id, slug: String(data.slug), name: String(data.name), description: String(data.description), url: data.url == null ? null : String(data.url), repositoryUrl: data.repositoryUrl == null ? null : String(data.repositoryUrl), isFeatured: data.isFeatured === true, displayOrder: Number(data.displayOrder ?? 0), publicationState: PublicationState.DRAFT }, update: { slug: String(data.slug), name: String(data.name), description: String(data.description), url: data.url == null ? null : String(data.url), repositoryUrl: data.repositoryUrl == null ? null : String(data.repositoryUrl), isFeatured: data.isFeatured === true, displayOrder: Number(data.displayOrder ?? 0), publicationState: PublicationState.DRAFT } });
